...
@section('content')
    ...
    <div class="row clearfix">
        @foreach($type_actualites as $type_act)
            <div class="col-xs-6 col-sm-4">
                <div class="card profile-card">
                    <div class="profile-header">
                        <h3 class="align-center">{{$type_act->titre_type_actualite}}</h2>
                    </div>
                    <div class="profile-body">
                        <div class="content-area align-center">
                            <p>{{$type_act->description_type_actualite}}</p>
                        </div>
                    </div>
                    <div class="profile-footer">
                        <button class="btn btn-danger btn-lg waves-effect btn-block" data-toggle="modal" data-target="#deleteConfirm{{$type_act->id_type_actualite}}">Delete</button>
                        <button class="btn btn-primary btn-lg waves-effect btn-block" data-toggle="modal" data-target="#updateConfirm{{$type_act->id_type_actualite}}">Edit</button>
                    </div>
                </div>
            </div>
            <div class="modal fade" id="deleteConfirm{{$type_act->id_type_actualite}}" tabindex="-1" role="dialog">
                <form method="POST" action="typeActualite/delete">
                    {{csrf_field()}}
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <input type="number" value="{{$type_act->id_type_actualite}}" name="id_type_actualite" class="hidden">
                            <h3>Voulez-vous supprimer le type d'action {{$type_act->titre_type_actualite}}</h3>
            ...
            <div class="modal fade" id="updateConfirm{{$type_act->id_type_actualite}}" tabindex="-1" role="dialog">
                <form method="POST" action="typeActualite/edit">
                    {{csrf_field()}}
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="form-group">
                            <div class="form-line">
                                <input type="text" value="{{$type_act->titre_type_actualite}}" name="titre_type_actualite" class="form-control" placeholder="Titre Type Actualite" maxlength="100">
                            </div>
                        </div>
                        <label for="description_type_actualite">Description Type Actualite</label>
                        <div class="form-group">
                            <div class="form-line">
                                <input type="text" value="{{$type_act->description_type_actualite}}" name="description_type_actualite" class="form-control" placeholder="Description Type Actualite" maxlength="250">
                            </div>
                        </div>
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-link waves-effect btn-primary" style="color: whitesmoke !important">Confirmer</button>
